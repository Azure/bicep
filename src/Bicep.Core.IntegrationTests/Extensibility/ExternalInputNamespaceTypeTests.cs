...
public class ExternalInputNamespaceTypeTests
{
    ...
    public static BinaryData GetExternalInputTypesTgz()
    {
        ...
        var extInputFunctionType = factory.Create(() => new FunctionType([
            new FunctionParameter("kind", factory.GetReference(stringType), "The kind parameter"),
            new FunctionParameter("config", factory.GetReference(anyType), "The config parameter"),
        ], factory.GetReference(anyType)));

        var fooType = factory.Create(() => new NamespaceFunctionType(
            "externalInput",
            factory.GetReference(extInputFunctionType), "Reads input from external tooling"));
        ...
        var index = new TypeIndex(
            ...
            new List<CrossFileTypeReference>() { new("types.json", factory.GetIndex(fooType)) },
            settings,
            null);
    ...
    public async Task ExternalInput_function_works()
    {
        ...
        var result = await CompilationHelper.RestoreAndCompile(services,
            ("main.bicep", new("""
extension '../extension.tgz' as externalInputShim

@export()
func scopeBinding(findKey string) any => externalInputShim.externalInput('ev2.scopeBinding', findKey)
""")),
