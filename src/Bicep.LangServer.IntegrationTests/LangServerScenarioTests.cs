...
```");
    ...
    public async Task Test_Issue13254() // https://github.com/Azure/bicep/issues/13254
    {
        ...
        var clientFactory = RegistryHelper.CreateMockRegistryClient(new RepoDescriptor("localhost", "test/foo", ["v1"]));
        async Task publish(string source)
            => await RegistryHelper.PublishModuleToRegistryAsync(
                new ServiceBuilder(),
                clientFactory,
                BicepTestConstants.FileSystem,
                new("br:localhost/test/foo:1.1", source, WithSource: false));
...
using 'br:localhost/test/foo:1.1'
