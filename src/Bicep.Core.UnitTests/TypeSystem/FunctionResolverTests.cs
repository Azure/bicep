    ...
    public class FunctionResolverTests
    {
        ...
        private static IEnumerable<object[]> GetSubstringTestCases()
        {
            ...
            return new[]
            {
                ...
                CreateRow(TypeFactory.CreateStringType(null, 5, null, LanguageConstants.SecureString.ValidationFlags), LanguageConstants.SecureString, LanguageConstants.Any, TypeFactory.CreateIntegerLiteralType(5)),
        ...
        public void TrimDropsMinLengthButPreservesMaxLengthAndFlags()
        {
            var returnType = EvaluateFunction("trim",
                new List<TypeSymbol> { TypeFactory.CreateStringType(10, 20, null, TypeSymbolValidationFlags.IsSecure) },
                new FunctionArgumentSyntax[] { new(TestSyntaxFactory.CreateVariableAccess("foo")) })
                .Type;

            returnType.Should().Be(TypeFactory.CreateStringType(minLength: null, 20, null, TypeSymbolValidationFlags.IsSecure));
        }
