...
namespace Bicep.Core.UnitTests.Modules
{
    ...
    public class OciArtifactModuleReferenceTests
    {
        ...
        {
            OciArtifactReference.TryParse(ArtifactType.Module, null, value, BicepTestConstants.BuiltInConfigurationWithAllAnalyzersDisabled, RandomFileUri()).IsSuccess(out var @ref, out var failureBuilder).Should().BeFalse();
            @ref.Should().BeNull();
