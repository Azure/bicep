...
public class ResourceVisitor
{
    ...
    public Dictionary<string, ProviderResourceTypes> GetAllResourceTypes()
    {
        var loader = new AzTypeLoader();
        IReadOnlyDictionary<string, CrossFileTypeReference> allResources = loader.LoadTypeIndex().Resources;

        var providers = new Dictionary<string, ProviderResourceTypes>();

        foreach (KeyValuePair<string, CrossFileTypeReference> resource in allResources)
        {
        ...
        _logger.LogInformation("Total Resource Providers: {ProviderCount}", providers.Count);

        return providers;
    }

    public TypesDefinitionResult LoadSingleVersion(string resourceProvider, string apiVersion)
    {
        var loader = new AzTypeLoader();
        ...
        var selectedResources = resources.Where(r => r.Key.ContainsInsensitively(resourceProvider) && r.Key.Contains(apiVersion)).OrderByAscendingOrdinalInsensitively(kvp => kvp.Key).ToList();
        var selectedResourceFunctions = resourceFunctions.Where(r => r.Key.ContainsInsensitively(resourceProvider)).ToList();
        ...
        foreach (KeyValuePair<string, CrossFileTypeReference> resource in selectedResources)
        {
            ResourceType resourceType = loader.LoadResourceType(resource.Value);
            FindTypesToWrite(typesToWrite, resourceType.Body);

            if (WriteComplexType(resourceType) is ResourceTypeEntity resourceTypeEntity)
            {
                result.ResourceTypeEntities.Add(resourceTypeEntity); // selectedResources is already sorted
            }
            else
            {
