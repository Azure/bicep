...
//@[10:63) [BCP335 (Warning)] The provided value can have a length as large as 35 and may be too long to assign to a target with a configured maximum length of 24. (CodeDescription: Core(https://aka.ms/bicep/core-diagnostics#BCP335)) |toLower('${i}one${uniqueString(resourceGroup().id)}')|
    location: resourceGroup().location
    sku: {
      name: 'Standard_LRS'
    }
...
//@[10:63) [BCP335 (Warning)] The provided value can have a length as large as 35 and may be too long to assign to a target with a configured maximum length of 24. (CodeDescription: Core(https://aka.ms/bicep/core-diagnostics#BCP335)) |toLower('${i}two${uniqueString(resourceGroup().id)}')|
    location: resourceGroup().location
    sku: {
      name: 'Standard_LRS'
    }
