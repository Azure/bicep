...
namespace Bicep.LanguageServer.Handlers
{
    public class BicepDisableLinterRuleHandler : ExecuteTypedCommandHandlerBase<DocumentUri, string, string>
    {
        ...
        {
            (string updatedBicepConfigSettingsFilePath, string updatedBicepConfigContents) = GetBicepConfigSettingsFilePathAndContents(documentUri, code, bicepConfigSettingsFilePath);

            File.WriteAllText(updatedBicepConfigSettingsFilePath, updatedBicepConfigContents);
        ...
        {
            if (!string.IsNullOrWhiteSpace(bicepConfigSettingsFilePath) && File.Exists(bicepConfigSettingsFilePath))
            {
                return (bicepConfigSettingsFilePath, DisableLinterRule(File.ReadAllText(bicepConfigSettingsFilePath), code));
            }
            else
            {
                string? directoryContainingSourceFile = Path.GetDirectoryName(documentUri.GetFileSystemPath());
