    ...
    public class CopilotComplete : IAutoComplete
    {
        ...
        public async Task<bool> GenerateCopilotCompletionsAsync(string schemaContent, string bicepContent)
        {
            ChatCompletionsOptions completionsOptions = new(this.GetChatMessages(schemaContent, bicepContent))
            {
                Temperature = 0.1f,
                //StopSequences = { "}" },
                MaxTokens = 1000, // TODO: See how much this actually needs to be when generating 3 possible autocompletions (will need to determine token count of various symbols in a policy definition JSON file). Compare w/token limit for entire prompt.
                ChoiceCount = 1
            };
            ...
            if (modelResponses != null && modelResponses.Count > 0)
            {
                ...
                Enumerable.DistinctBy(modelResponses, choice => choice.Message).ForEach(response => this.AddCompletionItem(new CompletionItem()
                {
                    ...
                    Label = response.Message.Content.Substring(this.contextRange.End.Character, 30),
                    InsertText = response.Message.Content,
                    TextEdit = new TextEdit { Range = new Range(this.contextRange.End, endPositionOfCompletion), NewText = response.Message.Content.Substring(this.contextRange.End.Character) },
                    InsertTextMode = InsertTextMode.AsIs,
                    Documentation = response.Message.Content,
                    Detail = "This response was generated by Bicep Copilot. As with any generative AI-based tool, the provided completions may be inaccurate. Please verify yourself that the autocompletion is correct and matches your specific use case.",
                }));
                return true;
            }
            else
