...
//@[08:039) [prefer-interpolation (Warning)] Use string interpolation instead of the concat function. (CodeDescription: Linter(https://aka.ms/bicep/linter/prefer-interpolation)) |concat(moduleName, moduleIndex)|
  params: {
...
//@[09:024) [BCP179 (Warning)] Unique resource or deployment name is required when looping. The loop item variable "zone" or the index variable "i" must be referenced in at least one of the value expressions of the following properties in the loop body: "name" (CodeDescription: Core(https://aka.ms/bicep/core-diagnostics#BCP179)) |duplicatedNames|
...
//@[09:034) [BCP179 (Warning)] Unique resource or deployment name is required when looping. The loop item variable "zone" or the index variable "i" must be referenced in at least one of the value expressions of the following properties in the loop body: "name" (CodeDescription: Core(https://aka.ms/bicep/core-diagnostics#BCP179)) |referenceToDuplicateNames|
...
resource indexedModuleCollectionDependency 'Microsoft.Network/frontDoors@2020-05-01' = [for (index, i) in range(0, length(regions)): {
  name: '${name}-${index}-${i}'
//@[08:031) [BCP334 (Warning)] The provided value can have a length as small as 4 and may be too short to assign to a target with a configured minimum length of 5. (CodeDescription: Core(https://aka.ms/bicep/core-diagnostics#BCP334)) |'${name}-${index}-${i}'|
  location: 'Global'
  properties: {
    backendPools: [
      {
