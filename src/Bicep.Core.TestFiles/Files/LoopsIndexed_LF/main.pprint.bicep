...
resource firstSet 'Microsoft.Storage/storageAccounts@2019-06-01' = [
  for (i, ii) in range(0, length(accounts)): {
    name: '${name}-set1-${i}-${ii}'
    location: resourceGroup().location
    kind: 'StorageV2'
    sku: {
...
resource secondSet 'Microsoft.Storage/storageAccounts@2019-06-01' = [
  for (i, iii) in range(0, length(accounts)): {
...
resource vnets 'Microsoft.Network/virtualNetworks@2020-06-01' = [
  for (vnetConfig, index) in vnetConfigurations: {
...
module moduleCollectionWithSingleDependency 'passthrough.bicep' = [
  for (moduleName, moduleIndex) in moduleSetup: {
    ...
    backendPools: [
      {
        ...
        properties: {
          backends: [
            for (index, i) in range(0, length(regions)): {
...
resource indexedResourceCollectionDependency 'Microsoft.Network/frontDoors@2020-05-01' = [
  for (index, i) in range(0, length(accounts)): {
    name: '${name}-${index}-${i}'
    location: 'Global'
    properties: {
      backendPools: [
