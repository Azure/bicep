...
namespace Bicep.Cli.IntegrationTests
{
    ...
    public class RestoreCommandTests : TestBase
    {
        ...
        public async Task Restore_should_succeed_for_files_matching_pattern(bool useRootPath)
        {
            ...
            var clientFactory = RegistryHelper.CreateMockRegistryClient(new RegistryHelper.RepoDescriptor("localhost", "test/foo", ["v1"]));
            await RegistryHelper.PublishModuleToRegistryAsync(
                new ServiceBuilder(),
                clientFactory,
                fileSystem,
                new("br:localhost/test/foo:1.1", """
output myOutput string = 'hello!'
""", WithSource: false));

            var contents = """
module mod 'br:localhost/test/foo:1.1' = {
  name: 'mod'
}
""";
...
";
        ...
        public async Task Restore_AggregateExceptionWithInnerRequestFailedExceptions_ShouldFail()
        {
            ...
            File.WriteAllText(compiledFilePath, @"module foo 'br:localhost/fake:v1'");
            ...
            var clientFactory = StrictMock.Of<IContainerRegistryClientFactory>();
            clientFactory
                .Setup(m => m.CreateAuthenticatedBlobClient(It.IsAny<CloudConfiguration>(), new Uri("https://localhost"), "fake"))
            ...
            using (new AssertionScope())
            {
                result.Should().Be(1);
                output.Should().BeEmpty();
                error.Should().Contain("main.bicep(1,12) : Error BCP192: Unable to restore the artifact with reference \"br:localhost/fake:v1\": One or more errors occurred. (Mock registry request failure 1.) (Mock registry request failure 2.)");
            }
        }
        ...
        public async Task Restore_RequestFailedException_ShouldFail()
        {
            ...
            using (new AssertionScope())
            {
                result.Should().Be(1);
                output.Should().BeEmpty();
                error.Should().Contain("main.bicep(1,12) : Error BCP192: Unable to restore the artifact with reference \"br:localhost/fake:v1\": Mock registry request failure.");
            }
        }
        ...
        public async Task Restore_bicepparam_should_fail_with_error_diagnostics_for_registry_failure(EmbeddedFile paramFile)
        {
            ...
            var clientFactory = StrictMock.Of<IContainerRegistryClientFactory>();
            clientFactory
                .Setup(m => m.CreateAuthenticatedBlobClient(It.IsAny<CloudConfiguration>(), new Uri("https://localhost"), "parameters/basic"))
                .Returns(client.Object);
            ...
            result.Stderr.Should().Contain("main.bicepparam(1,7) : Error BCP192: Unable to restore the artifact with reference \"br:localhost/parameters/basic:v1\": Mock registry request failure.");
        }
