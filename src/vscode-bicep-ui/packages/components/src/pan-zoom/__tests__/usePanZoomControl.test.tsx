...
describe("usePanZoomControl", () => {
  it("should zoom in", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("pan-zoom transform spy")).toHaveStyle({ transform: "translate(0px, 0px) scale(1.2)" });
    });
  });
  ...
  it("should zoom out", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("pan-zoom transform spy").style.transform).toMatch(/^translate\(0px, 0px\) scale\(0.83.+\)$/);
    });
  });
