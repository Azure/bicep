    ...
    <FormSection title="Operations">
      <VSCodeDataGrid>
        <VSCodeDataGridRow rowType="header">
          ...
          <VSCodeDataGridCell gridColumn="2" cellType="columnheader">State</VSCodeDataGridCell>
          <VSCodeDataGridCell gridColumn="3" cellType="columnheader">Error</VSCodeDataGridCell>
        </VSCodeDataGridRow>
        {result.operations.map(operation => (
          <VSCodeDataGridRow key={operation.resourceName} style={isFailed(operation) ? { background: "rgba(255, 72, 45, 0.3)" } : {}}>
            ...
            <VSCodeDataGridCell gridColumn="3">{operation.error ? getPreformattedJson(operation.error) : ''}</VSCodeDataGridCell>
          </VSCodeDataGridRow>
        ))}
      </VSCodeDataGrid>
    </FormSection>
      ...
      <VSCodeDataGrid>
        ...
        {Object.keys(result.deployment.outputs).map(name => (
          <VSCodeDataGridRow key={name}>
            ...
            <VSCodeDataGridCell gridColumn="2">{getPreformattedJson(result.deployment.outputs[name])}</VSCodeDataGridCell>
          </VSCodeDataGridRow>
        ))}
      </VSCodeDataGrid>
    </FormSection>
