...
export class AzurePickers extends Disposable {
  ...
  public async EnsureSignedIn(context: IActionContext): Promise<void> {
    ...
    if ((await this.getAllSubscriptions()).length === 0) {
      // No subscriptions found, user may need to sign in with a specific tenant
      context.telemetry.properties.subscriptionCount = "0"; //asdfg
      this.outputChannelManager.appendToOutputChannel(`No currently-accessible subscriptions found. ${await this.getTenantsStatusString()}`);

      const tenant = await this.pickTenant(context);
      context.telemetry.properties.subscriptionCount = "0"; //asdfg
      await this.vsCodeAzureSubscriptionProvider.signIn(tenant.tenantId); //asdfg what if cancelled?
    }
  }

  // Throws if user cancels asdfg
  private async pickTenant(context: IActionContext): Promise<TenantIdDescription> {
    const tenants = await this.getTenantsStatus();
    if (tenants.length === 0) {
  ...
  public async pickSubscription(context: IActionContext): Promise<AzureSubscription> {
    ...
    if (subscriptions.length === 0) {
      throw new Error(`No currently-accessible subscriptions found. ${await this.getTenantsStatusString()}`);
    }
  ...
  public async pickManagementGroup(context: IActionContext): Promise<ManagementGroupInfo> {
    ...
    try {
      ...
      throw new Error(
        `You might not have access to any management groups. Please create one in the Azure portal and try to deploy again.  Error: ${parseError(err).message}. ${await this.getTenantsStatusString()}`,
      );
    }
  ...
  private async getTenantsStatus(): Promise<{ tenant: TenantIdDescription; isSignedIn: boolean }[]> {
  ...
  private async getTenantsStatusString(): Promise<string> {
    try {
      const tenants = await this.getTenantsStatus();
      const signInStatus = tenants.map(tenant => `${tenant.tenant.tenantId} (${tenant.isSignedIn ? "signed in" : "signed out"})`);
