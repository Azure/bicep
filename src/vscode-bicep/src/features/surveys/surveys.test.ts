...
import { createActionContext } from "../../../test-support/action-context";
...
describe("Survey", () => {
  ...
  test("postpones for 180 days when the user chooses don't ask again", async () => {
    ...
    await mocks.survey.checkShowSurvey(createActionContext(), now);
  ...
  test("doesn't prompt when the survey is unavailable", async () => {
    ...
    await mocks.survey.checkShowSurvey(createActionContext(), new Date());
