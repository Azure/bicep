...
import { ExtractToModuleParams, extractToModuleRequestType, ExtractToModuleResult } from "./protocol";
...
export class ExtractToModuleCommand implements Command {
  ...
  public async execute(documentUri?: Uri): Promise<void> {
    ...
    await workspace.fs.createDirectory(Uri.file(path.dirname(resolvedTarget.fsPath)));
