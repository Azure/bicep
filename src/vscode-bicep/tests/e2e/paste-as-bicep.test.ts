...
import { getE2eLogPath } from "./utils/e2e-log";
import { waitUntil } from "./utils/polling";
import { normalizeBicepText } from "./utils/text-normalization";
...
describe("pasteAsBicep", (): void => {
  ...
  async function runTest(
    ...
    if (typeof expected.bicep === "string") {
      expect(normalizeBicepText(buffer)).toBe(normalizeBicepText(expected.bicep));
    }
    ...
    async function waitForPasteAsBicep(expectedSubstring: string): Promise<void> {
      await waitUntil(() => isReady(), {
        description: `the paste operation to log ${expectedSubstring}`,
        interval: 100,
        timeoutMs: 4000,
      });
      if (!isReady()) {
