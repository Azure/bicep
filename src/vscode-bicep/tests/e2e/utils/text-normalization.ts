...
export function normalizeBicepText(s: string, spacesPerTab = 2): string {
...
function normalizeIndentation(s: string, spacesPerTab: number): string {
  ...
  const commonIndent =
    contentLines.length === 0 ? 0 : Math.min(...contentLines.map((line) => line.length - line.trimStart().length));
  const relativeIndents = contentLines.map((line) => line.length - line.trimStart().length - commonIndent);
