...
namespace Bicep.Cli.Commands
{
    public class LintCommand : ICommand
    {
        ...
        public async Task<int> RunAsync(LintArguments args)
        {
            ...
            {
                var compilation = await compilationService.CompileAsync(inputPath, skipRestore: false);
                var diagnosticLevel = args.IgnoreWarnings ? DiagnosticLevel.Error : DiagnosticLevel.Warning;

                if (diagnosticLogger.ErrorCount > 0 || compilation.GetAllDiagnosticsByBicepFile().Any(c => c.Value.Any(d => d.Level >= diagnosticLevel)))
                {
                    return 1;
                }
