...
public class SourceArchive : IDisposable
{
    ...
    public IEnumerable<(FileMetadata Metadata, string Contents)> GetSourceFiles() //asdfg don't retrieve everything immediately
    {
        ...
        foreach (var entry in metadata.SourceFiles.OrderBy(e => e.Path).ThenBy(e => e.ArchivedPath))
        {
