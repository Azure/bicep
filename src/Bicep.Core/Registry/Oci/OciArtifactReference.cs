...
namespace Bicep.Core.Registry.Oci
{
    public class OciArtifactReference : ArtifactReference, IOciArtifactReference, IExtensionArtifactReference
    {
        ...
        public OciArtifactReference(IFileExplorer fileExplorer, RootConfiguration configuration, ArtifactType type, IOciArtifactAddressComponents artifactIdParts) :
            base(configuration, OciArtifactReferenceFacts.Scheme)
        {
            Type = type;
            AddressComponents = artifactIdParts;
            lazyBicepRegistryModuleArtifact = new(() => new(this.AddressComponents, this.Configuration.GetCacheRootDirectory(fileExplorer)));
            lazyBicepRegistryExtensionArtifact = new(() => new(this.AddressComponents, this.Configuration.GetCacheRootDirectory(fileExplorer)));
        }

        public OciArtifactReference(IFileExplorer fileExplorer, RootConfiguration configuration, ArtifactType type, string registry, string repository, string? tag, string? digest) :
            base(configuration, OciArtifactReferenceFacts.Scheme)
        {
            switch (tag, digest)
            {
        ...
        public static ResultWithDiagnosticBuilder<OciArtifactReference> TryParse(IFileExplorer fileExplorer, RootConfiguration configuration, ArtifactType type, string? aliasName, string unqualifiedReference)
        {
            ...
            {
                return new(new OciArtifactReference(fileExplorer, configuration, type, components.Registry, components.Repository, components.Tag, components.Digest));
            }
            else
            {
