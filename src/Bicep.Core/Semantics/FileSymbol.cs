        ...
        private sealed class DuplicateIdentifierValidatorVisitor : SymbolVisitor
        {
            ...
            private void ValidateScope(ILanguageScope scope)
            {
                ...
                this.Diagnostics.AddRange(
                    ...
                    .Select(decl => DiagnosticBuilder.ForPosition(decl.DeclaringProvider.Specification).NamespaceMultipleDeclarations(decl.DeclaringProvider.Specification.Identifier)));
            }
            ...
            private static IEnumerable<ProviderNamespaceSymbol> FindDuplicateNamespaceImports(IEnumerable<ProviderNamespaceSymbol> symbols)
            {
                ...
                return typeBySymbol
                    ...
                    .GroupBy(kvp => kvp.Key.DeclaringProvider.Specification.Identifier, LanguageConstants.IdentifierComparer)
                    .Where(group => group.Count() > 1)
