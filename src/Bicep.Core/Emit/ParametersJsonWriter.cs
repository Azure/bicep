...
public class ParametersJsonWriter
{
        ...
        IDictionary<FunctionCallSyntaxBase, ExternalInputInfo> externalInputInfo)
    {
        emitter.EmitObjectProperty("externalInputDefinitions", () =>
        {
            ...
            {
                var externalInputInfo = reference.Value;

                emitter.EmitObjectProperty(externalInputInfo.DefinitionKey, () =>
                {
                    emitter.EmitProperty("kind", externalInputInfo.Kind);
                    if (externalInputInfo.Config is { } config)
                    {
