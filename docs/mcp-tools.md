...
- `list_resource_types`: Lists all available resource types for a specific provider namespace. Supports Azure (az) resource types by default, and extension resource types (e.g., Microsoft Graph) when `extensionName` and `extensionVersion` parameters are provided. The return value is a list of resource types including their API version, e.g. `Microsoft.KeyVault/vaults@2024-11-01` or `Microsoft.Graph/applications@v1.0`.
- `get_resource_type_schema`: Gets the schema for a specific resource type and API version. Supports Azure resource types by default, and extension resource types when `extensionName` and `extensionVersion` parameters are provided.
- `list_published_extensions`: Lists available Bicep extensions (e.g., Microsoft Graph) with their dynamically-discovered version tags from MCR. Use this to discover extension names and versions for use with `list_resource_types` and `get_resource_type_schema`.
