...
resource nic 'Microsoft.Network/networkInterfaces@2018-11-01' = [for i in range(0, rdshNumberOfInstances): {
  ...
  properties: {
    ...
    networkSecurityGroup: (empty(networkSecurityGroupId) ? any(null) : json('{"id": "${nsgId}"}'))
  }
  dependsOn: [
    NSG
  ]
...
resource vm 'Microsoft.Compute/virtualMachines@2018-10-01' = [for i in range(0, rdshNumberOfInstances): {
  ...
  properties: {
    ...
    availabilitySet: ((availabilityOption == 'AvailabilitySet') ? vmAvailabilitySetResourceId : any(null))
    osProfile: {
