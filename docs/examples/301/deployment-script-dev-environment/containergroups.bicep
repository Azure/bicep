    ...
    containers: [
      {
        name: '${name}cg'
        properties: {
          image: type == 'AzureCLI' ? azcliImage : type == 'AzurePowerShell' ? azpwshImage : any(null)
          resources: {
            requests: {
              cpu: 1
              memoryInGB: 2
          ...
          command: type == 'AzureCLI' ? azcliCommand : type == 'AzurePowerShell' ? azpwshCommand : any(null)
        }
      }
    ]
    osType: 'Linux'
