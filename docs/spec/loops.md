...
The example below demonstrates a nested loop combined with filters at each loop.

```
resource parentResources '...' = [for parent in parents where (parent.enabled) {
  name: parent.name
  properties: {
