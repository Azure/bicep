...
Enables the `this` namespace for accessing the current resource instance. The `this` namespace is only discoverable in resource bodies. Currently, `this.exists()`, `this.existingResource()`, and `this.existingProperties()` are available for usage. `this.exists()` returns a bool indicating the existence of the current resource. `this.existingProperties()` returns null if the resource does not exist and returns the resource's properties if the resource exists. `this.existingResource()` returns null if the resource does not exist and returns the full resource if the resource exists. For example:
```
resource usingThis 'Microsoft...' = {
  ...
  properties: {
    ...
    property2: this.existingProperties().?property2
